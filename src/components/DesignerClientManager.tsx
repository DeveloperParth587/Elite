...
export function DesignerClientManager() {
  ...
  const handleAddClient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to initialize client account.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        {showForm ? (
          ...
          >
            <Card className="rounded-[40px] border-brand-border bg-white shadow-2xl overflow-hidden max-w-xl mx-auto border-none ring-1 ring-brand-border">
              <CardHeader className="bg-brand-sidebar/20 border-b border-brand-border p-10 text-center">
                <div className="w-20 h-20 bg-white rounded-[28px] shadow-sm flex items-center justify-center mx-auto mb-6 border border-brand-border">
                   <UserPlus className="h-10 w-10 text-brand-olive" />
                </div>
                <CardTitle className="text-3xl font-serif text-brand-ink italic">Client Onboarding</CardTitle>
                <CardDescription className="font-medium text-neutral-400 mt-2 text-base">Initialize a secure portal for your new client.</CardDescription>
              </CardHeader>
              <form onSubmit={handleAddClient}>
                <CardContent className="p-10 space-y-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-[10px] text-neutral-400 italic px-2">Passwords must be at least 6 characters long for security compliance.</p>
                  </div>
              ...
              clients.map((client) => (
                <Card key={client.id} className="group rounded-[32px] border-brand-border bg-white p-6 hover:shadow-2xl hover:shadow-brand-olive/10 hover:border-brand-olive/30 transition-all duration-500 overflow-hidden ring-1 ring-brand-border/10">
                   <div className="flex items-start justify-between mb-8">
                      ...
                      <Badge className="bg-emerald-50 text-emerald-600 border-none font-black text-[9px] uppercase tracking-widest px-3 py-1">Active Client</Badge>
                   </div>
                   <div className="space-y-1">
                      <h3 className="text-lg font-bold text-brand-ink truncate">{client.email}</h3>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-clay leading-none">Studio Member</p>
                   </div>
                   <div className="mt-8 pt-6 border-t border-brand-border flex items-center justify-between">
                      <div className="flex flex-col">
                         <span className="text-[9px] font-bold text-neutral-300 uppercase tracking-widest">Client Since</span>
                         <span className="text-xs font-bold text-neutral-500">{new Date(client.created_at).toLocaleDateString()}</span>
                      </div>
